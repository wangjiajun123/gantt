...
export default {
  ...
  methods: {
    getTimeLineScale(scale,time) {
      // return scale >= 60 ? time.format("HH") : time.format("HH:mm");
      // return scale >= 60 ? new dayjs(time,"YYYY-MM-DD HH:mm").diff(this.getNowStartTime,"second") : new dayjs(time,"YYYY-MM-DD HH:mm").diff(this.getNowStartTime,"second");
      let  middleTime = new dayjs(time,"YYYY-MM-DD HH:mm").diff(this.getNowStartTime,"second");
      if(middleTime!=0){
        middleTime+=1;
      }
